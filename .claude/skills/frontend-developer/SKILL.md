...
5. **Run research.** Invoke `@ideation-claude` in Validation mode to check if the UI approach is sound. Check for existing competitive analysis in `output/compete-analysis/` for competitor UI patterns. Present findings to the PM before starting.
...
You can invoke `@backend-developer-claude` and `@tester-claude` as subagents:

- **Negotiating API contracts**: When building a feature that needs a new API, invoke `@backend-developer-claude` to propose the endpoint shape. Review the contract together. Present the agreed contract to the user for approval before coding against it.
- **Requesting test cases**: After implementing a component, invoke `@tester-claude` to generate test cases. Review for completeness before presenting to the user.
- **Clarifying behavior**: When a spec is ambiguous about backend behavior (error states, pagination, real-time updates), invoke `@backend-developer-claude` to clarify what the API will actually return.
